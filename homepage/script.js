  ...
  if (typeof gsap !== 'undefined' && preloader) {
    gsap.to(preloader, {
      ...
      onComplete: () => {
        ...
        // remove loading markers to re-enable scroll
        ...
        if (smoothWrapperEl) smoothWrapperEl.classList.remove('loading');
        const lenisContainer = document.querySelector('.lenis-scroll-container');
        if (lenisContainer) lenisContainer.classList.remove('loading');
        animateWords();
      }
    });
  } else if (preloader) {
...
if (shouldPlayPreloader) {
  ...
  // Also mark html and common smooth containers so CSS can reliably lock scrolling
  try { document.documentElement.classList.add('loading'); } catch (e) {}
  const smoothWrapperEl = document.getElementById('smooth-wrapper');
  if (smoothWrapperEl) smoothWrapperEl.classList.add('loading');
  const lenisContainer = document.querySelector('.lenis-scroll-container');
  if (lenisContainer) lenisContainer.classList.add('loading');
  ...
  // remove loading markers in case they were set
  document.body.classList.remove("loading");
  try { document.documentElement.classList.remove('loading'); } catch (e) {}
  const smoothWrapperEl = document.getElementById('smooth-wrapper');
  if (smoothWrapperEl) smoothWrapperEl.classList.remove('loading');
